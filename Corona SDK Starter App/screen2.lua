...
newsGroup:insert(bodyBackground3)
 ...
 local textDate = display.newText("screen2.lua", 0, 0, native.systemFontBold, 16)
...
local textNewsTitle = display.newText("Samples", 0, 0, native.systemFontBold, 24)
textNewsTitle:setTextColor(0, 0, 0)
textNewsTitle.x = display.contentWidth * .02
textNewsTitle.y = display.contentHeight * .33
newsGroup:insert(textNewsTitle)     

local buttonToolPress = function( event )
end

local buttonToolRelease = function( event )
	if event.phase == "ended" then
    director:changeScene ("screen2b", "overFromRight" )
    end
end

local buttonTool = ui.newButton{
  default = "buttonBigWhite.png",
  over = "buttonBigWhite_over.png",
	onPress = buttonToolPress,
	onRelease = buttonToolRelease,
  text = "ScrollView sample",
...
mainView:insert(buttonTool)
buttonTool.x = display.contentWidth * .5 
buttonTool.y = display.contentHeight * .5
